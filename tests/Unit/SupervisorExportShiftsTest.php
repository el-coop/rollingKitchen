...
class SupervisorExportShiftsTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$workFunction = WorkFunction::factory()->create([
			'workplace_id' => $this->workplace->id
		]);
		$this->shifts = Shift::factory(2)->create(['workplace_id' => $this->workplace->id]);

		$this->shifts->each(function ($shift) use ($workFunction) {
			Worker::factory(2)->create()->each(function ($worker) use ($shift, $workFunction) {
