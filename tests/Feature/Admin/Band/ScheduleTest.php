...
class ScheduleTest extends TestCase {
	...
	public function test_admin_can_access_schedule_page() {
        $dbSchedules = BandSchedule::select('date_time', 'end_time', 'stage_id as stage', 'band_id as band', 'payment', 'approved')->get()->groupBy('date_time');
        $this->actingAs($this->admin)->get(action('Admin\BandController@schedule'))->assertSuccessful()
			...
			->assertViewHas('schedules', function ( $schedule) use ($dbSchedules){
                foreach ($dbSchedules as $key => $dbSchedule){
                    return $schedule->get($key)->first()->band == $dbSchedule->first()->band &&
                        $schedule->get($key)->first()->stage == $dbSchedule->first()->stage &&
                        $schedule->get($key)->first()->payment == $dbSchedule->first()->payment;
                }
            });
	}
