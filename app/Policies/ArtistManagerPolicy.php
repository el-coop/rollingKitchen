<?php

namespace App\Policies;

use App\Models\Admin;
use App\Models\Developer;
use App\Models\User;
use App\Models\ArtistManager;
use Illuminate\Auth\Access\HandlesAuthorization;

class ArtistManagerPolicy {
	use HandlesAuthorization;

	public function before($user, $ability) {
		if ($user->user_type == Developer::class) {
			return true;
		}
	}

	/**
	 * Determine whether the user can view the artist manager.
	 *
	 * @param  \App\Models\User $user
	 * @param  \App\Models\ArtistManager $artistManager
	 * @return mixed
	 */
	public function view(User $user, ArtistManager $artistManager) {
		//
	}

	/**
	 * Determine whether the user can create artist managers.
	 *
	 * @param  \App\Models\User $user
	 * @return mixed
	 */
	public function create(User $user) {
		return $user->user_type == Admin::class;
	}

	/**
	 * Determine whether the user can update the artist manager.
	 *
	 * @param  \App\Models\User $user
	 * @param  \App\Models\ArtistManager $artistManager
	 * @return mixed
	 */
	public function update(User $user, ArtistManager $artistManager) {
		return $user->user_type == Admin::class;

	}

	/**
	 * Determine whether the user can delete the artist manager.
	 *
	 * @param  \App\Models\User $user
	 * @param  \App\Models\ArtistManager $artistManager
	 * @return mixed
	 */
	public function delete(User $user, ArtistManager $artistManager) {
		return $user->user_type == Admin::class;

	}

	/**
	 * Determine whether the user can restore the artist manager.
	 *
	 * @param  \App\Models\User $user
	 * @param  \App\Models\ArtistManager $artistManager
	 * @return mixed
	 */
	public function restore(User $user, ArtistManager $artistManager) {
		//
	}

	/**
	 * Determine whether the user can permanently delete the artist manager.
	 *
	 * @param  \App\Models\User $user
	 * @param  \App\Models\ArtistManager $artistManager
	 * @return mixed
	 */
	public function forceDelete(User $user, ArtistManager $artistManager) {
		//
	}
}
