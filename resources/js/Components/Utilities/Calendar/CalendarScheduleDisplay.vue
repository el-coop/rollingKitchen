<template>
    <div class="tags">
		<span v-for="(entry, index) in entries" class="is-hidden">
			...
			<input :key="`end_time_${index}`" type="hidden"
                   :value="entry.end_time"
                   :name="`calendar[${dateTime}][${index}][endTime]`">
		</span>
        <div v-show="!filter || filter == entry.band" class="tags has-addons" v-for="entry in entries"
             :key="entry.stage"
